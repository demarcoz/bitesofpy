...
def create_parser():
    ...
    group.add_argument("--add", "-a", nargs=3, help="add item providing name (str), price(int), and craving (bool)")
    group.add_argument("--list", "-l", help="show items in cart", action="store_true")
    group.add_argument("--delete", "-d", nargs=1, help="delete a product by name (str)")
    group.add_argument("--search", "-s", nargs=1, help="search items by name")
...
def handle_args(args=None, cart=None):
    ...
    if args.add:
        craving = True if args.add[2] == "True" else False
        article = Item(
            product=args.add[0],
