...
def operas_both_at_premiere(guest, composer):
    ...
    for opera in operas:
        ...
        opera_date = datetime.strptime(opera.date, "%d %B %Y")

        guest_born = datetime.strptime(composers[guest].born, "%d %B %Y")
        guest_died = datetime.strptime(composers[guest].died, "%d %B %Y")

        composer_died = datetime.strptime(composers[composer].died, "%d %B %Y")

        if guest_born < opera_date < guest_died:
            if opera_date > composer_died:
                continue
            available_operas.append(opera.play)

    return available_operas